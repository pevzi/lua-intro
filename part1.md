...
Переменную можно воспринимать как нечто имеющее имя и хранящее некоторое значение. Операция присваивания "кладёт" в указанную переменную значение выражения, располагающегося справа от знака равенства.
...
То есть, в сложном выражении используемые операторы будут выполняться именно в этом порядке. Если вам нужно изменить порядок выполнения операторов (или если вы просто забыли, кто за кем следует), то используйте круглые скобки:
...
- они хранятся непосредственно в переменных и копируются при присваивании;
- они неизменяемы, то есть, любая операция над этими значениями приводит к созданию нового значения.
...
- объекты в памяти могут быть изменены без изменения переменных, хранящих указатель на него.

Обратите внимание: присваивание _не изменяет само значение_, оно изменяет _переменную_, копируя в неё это значение. Да, по сути, присваивание всегда копирует значение, просто в случае объектных типов этим значением является указатель на объект.

Из определения объектных типов также следует то, что при сравнении значений таких типов производится сравнение _указателей_ на объекты, а не самих объектов. Это означает, что два значения в этом случае будут равны только если они указывают на один и тот же объект. В следующей части это свойство будет наглядно рассмотрено на примере таблиц.

Строки являются в некотором смысле особенными касательно реализации. "Снаружи" они ведут себя как примитивные значения (неизменяемы и сравниваются "по смыслу"), но "внутри" они реализованы как указатель на последовательность байт. Также имеет место оптимизация, называемая string interning: при создании строк Lua следит за тем, чтобы они хранились в памяти только в одном экземпляре. Если оказывается, что создаваемая строка уже существует в памяти, то используется уже имеющаяся. Таким образом, сравнение строк происходит очень быстро, так как оно по сути сводится к сравнению указателей на память.

Стоит также отметить, что вручную выделять память под объекты и строки, а также впоследствии очищать её не нужно, всё это делается автоматически. Объекты и строки, на которые не указывает ни одна переменная, считаются недоступными и через некоторое время автоматически удаляются из памяти сборщиком мусора.
