...
- при присваивании производится копирование значения в указанную переменную (за исключением типа string - см. примечание ниже).
...
Заметьте, что вручную выделять память под объекты и строки, а также впоследствии очищать её не нужно, всё это делается автоматически. Объекты и строки, до которых нельзя добраться через доступные переменные, спустя некоторое время автоматически удаляются из памяти сборщиком мусора.

TODO: может, предупредить, что над указателями нельзя выполнять арифметические операции
TODO: может, как-то дать понять, что при присваивании не создаётся ссылка на переменную, как в плюсах с &
