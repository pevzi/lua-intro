...
Инструкции switch из Си здесь нет.
...
Шаг может быть отрицательным, чтобы считать в обратную сторону, но тогда нужно указывать соответствующие "от" и "до", чтобы "от" было больше.
...
Цикл с pairs интересен тем, что при его использовании можно без опаски изменять значения существующих элементов таблицы (в том числе на nil), и это не приведёт к непредсказуемым последствиям при обходе. Однако, добавление новых элементов в таблицу во время её обхода не рекомендуется, так как в этом случае поведение не определено.

Кстати, если вы замечаете, что вам часто приходится определять наличие элемента в массиве, каждый раз пробегая циклом по всем элементам, или даже периодически удалять эти элементы из массива, то стоит подумать, действительно ли вам требуется хранить элементы в определённом порядке. Если вам не важен порядок, и элементы являются гарантированно уникальными, то гораздо эффективнее будет хранить их как ключи в таблице, используя в качестве значений любое истинное значение. Например:
...
Да, кстати, любой цикл (while, repeat-until, for) можно принудительно прервать, используя инструкцию break:
...
while true do -- бесконечный цикл
    io.stdout:write("your guess: ") -- используем write вместо print, чтобы не добавлялся перенос строки
    guess = io.stdin:read("*n") -- считываем число со стандартного потока ввода
