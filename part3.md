...
Заметьте, что присваивание в Lua не является выражением, как в Си-образных языках. Это исключает распространённую ошибку с использованием одинарного знака равенства вместо двойного:

```lua
if lives = 0 then -- ошибка синтаксиса
    gameOver()
end
```
...
2. Метка не видна внутри вложенных в блок функций, то есть, нельзя прыгнуть в тело другой функции.
3. Нельзя прыгнуть в область видимости локальной переменной, которой не существовало на момент прыжка.

(Функции и локальные переменные будут рассмотрены позже.)
