...
Тот факт, что функция является значением, также говорит о том, что функция сама по себе имени не имеет. Поэтому словосочетания вроде "функция print" следует воспринимать как "функция, на которую указывает переменная print".
...
Ещё одно полезное применение замыканий: функции, обращающиеся к некоторым внешним переменным, можно передавать в качестве аргументов там, где требуется callback (функция обратного вызова). При вызове такой функции произойдёт предсказуемое обращение к требуемым переменным вне зависимости от того, где будет совершён её вызов. [Здесь](http://www.lua.org/pil/6.1.html) есть ещё несколько примеров.
