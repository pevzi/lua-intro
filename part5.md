...
Инструкцию return так же, как и break, можно использовать только в самом конце блока (во всех версиях Lua 5.x).
...
Использование вызова функции в инструкции return также работает ожидаемым образом и по тем же правилам:
...
Кстати, если в функцию нужно передать только один строковый или табличный аргумент в виде литерала (т.е. в виде указанного прямо в исходном коде значения), при вызове функции круглые скобки можно не писать:
...
TODO: пример с позиционными аргументами в дополнение к ключевым?
TODO: два варианта:
    - позиционные принимаются как обычно, ключевые - как ещё один аргумент-таблица
    - все аргументы принимаются как таблица, но позиционные - под целочисленными ключами
...
TODO: переписать всё и раскрыть применимость ООП более понятно
...
TODO: придумать интересный пример с полиморфизмом из геймдева
TODO: инкапсуляция, согласно одному из определений, - объединение данных и методов для работы с ними в одну сущность
TODO: тут лучше акцентировать внимание на ООП без классов (ООП - это про объекты, а не про классы)
TODO: а ваще в конце-то концов, у меня не книга про ООП, зачем мне это всё так фундаментально пояснять?
...
```lua
local player = {x = 10, y = 5, health = 80, maxHealth = 100}
...
В такой записи неудобно то, что между объектом player и функцией healPlayer нет никакой явной связи. Если потребуется передать куда-то игрока, то придётся как-то отдельно дать доступ и к функции healPlayer, чтобы там могли знать, как его лечить. Более того, она может быть далеко не единственной функцией, работающей с данным объектом. А что если нам, например, потребуется унифицированно работать с набором объектов, которые должны по-разному реагировать на одни и те же команды?

Вместо того, чтобы хранить вышеуказанную функцию в отдельной локальной переменной, можно сохранить её в самой таблице player в качестве значения под ключом heal. Тогда можно получить что-то похожее на объект с методами, т.е. сущность, объединяющую в себе и данные, и функции для работы с ними:
...
TODO: можно определять функции заранее, а потом в конструкторе присваивать их полям таблицы
TODO: можно даже присваивать разные функции с одинаковым интерфейсом, чтоб типа фабрики объектов и полиморфизм
TODO: + всё-таки привести пример конструкторов на основе замыканий, но предупредить, что создание замыканий - довольно дорогая операция
