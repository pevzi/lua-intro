...
Так можно не бояться, что какая-то из глобальных переменных случайно "утечёт" в \_G, ибо при присваивании переменные в любом случае сохраняются в P. Но при этом возможность явно обратиться напрямую к \_G остаётся.
...
(Да, переменная \_G всё ещё не используется языком и не влияет на механизм обращения к глобальным переменным.)
...
do
    ...
    function foo()
        print(v, pairs) -- _ENV.print(_ENV.v, _ENV.pairs)
